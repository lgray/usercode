...
for base, job in jobs :
    command = 'python scripts/filter.py  --filesDir root://eoscms/%s/%s/ --fileKey tree.root --outputDir /tmp/jkunkle/OutputS2NoEleVeto_2013_11_03/%s --outputFile tree.root --treeName ggNtuplizer/EventTree --module scripts/ConfWgamgamReco.py --enableKeepFilter --nFilesPerJob 5 --storagePath /eos/cms/store/user/jkunkle/Wgamgam/OutputS2NoEleVetoLowLepPt_2013_11_08/%s --nproc 8' %( base, job, job, job ) 
    print command
    os.system(command)
