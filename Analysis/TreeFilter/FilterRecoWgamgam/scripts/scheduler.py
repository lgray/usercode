...
for base, job in jobs :
    command = 'python scripts/filter.py  --filesDir root://eoscms/%s/%s/ --fileKey tree.root --outputDir /tmp/jkunkle/Output/%s --outputFile tree.root --treeName ggNtuplizer/EventTree --module scripts/ConfWgamgamReco.py --enableKeepFilter --nFilesPerJob 5 --storagePath /eos/cms/store/user/jkunkle/Wgamgam/Output/%s --nproc 8' %( base, job, job, job ) 
    os.system(command)
